...
spotify_data %>%
  ...
  mutate(fraction = total_streams/sum(total_streams),
         ...
         label = total_streams) %>%
  ggplot() +
  geom_rect(aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 3, fill = track), color = "black") +
  coord_polar(theta = "y", start = -pi/2) +
  xlim(c(2, 4)) +
  geom_text(x = 2, y = 2, label = "Lorde Summer", size = 5, family = "Bowlby", fontface = "bold") +
  geom_text(aes(x = 3.5, y = position, label = label), size = 6, color = "black", family = "Manrope") +
